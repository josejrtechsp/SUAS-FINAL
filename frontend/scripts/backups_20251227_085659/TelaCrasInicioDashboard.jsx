...
export default function TelaCrasInicioDashboard({
  ...
  async function fetchJson(url) {
    ...
    if (!res.ok) {
      const msg = json?.detail || `Falha (HTTP ${res.status})`;
      throw new Error(msg);
    }
    return json;
  }
  ...
  async function load() {
    ...
    setErro(null);
    try {
      // 1) overview principal (preferir dashboard/overview, fallback relatorios/overview)
      let ov = null;
      try {
        ov = await fetchJson(withQuery("/cras/dashboard/overview"));
      } catch {
        ov = await fetchJson(withQuery("/cras/relatorios/overview"));
      }
      setData(ov);

      // 2) tarefas/SLA por técnico
      try {
        const tr = await fetchJson(withQuery("/cras/tarefas/resumo"));
        setTarefasResumo(tr);
      } catch {
      ...
      {erro ? (
        <div className="card" style={{ padding: 12, borderRadius: 14, marginBottom: 12 }}>
          ...
          <div className="texto-suave" style={{ marginTop: 6 }}>
            Se a mensagem for “Token de autenticação…”, faça login novamente.
          </div>
        </div>
      ) : null}
