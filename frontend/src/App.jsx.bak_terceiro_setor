...
import TelaGestaoRegional from "./TelaGestaoRegional";
import PageHero from "./components/PageHero";
import { getSuasUpdatesForCase } from "./domain/suasEncaminhamentosStore.js";
...
/* ===============================
   FAILSAFE SHARED UI (module-scope)
   Evita ReferenceError em sub-telas quando algo for usado fora de props.
   =============================== */
let suasCaseUI = { timeline: [], metroRegistros: [] };
let linhaMetroUI = null;

function App({ onLogout, usuarioLogado }) {
  ...
  // ✅ Integração SUAS (localStorage): devolutivas/cobranças aparecem dentro do caso PopRua (metrô + card)
  const suasCaseUIMemo = useMemo(() => {
    if (!selectedCaso) return { timeline: [], metroRegistros: [] };
    return getSuasUpdatesForCase({ modulo: "POPRUA", casoId: selectedCaso?.id, pessoaId: selectedCaso?.pessoa_id });
  }, [selectedCaso?.id, selectedCaso?.pessoa_id]);

  const linhaMetroUIMemo = useMemo(() => {
    if (!linhaMetro) return null;
    const extraRegs = Array.isArray(suasCaseUIMemo?.metroRegistros) ? suasCaseUIMemo.metroRegistros : [];
    ...
    const prefer = String(selectedCaso?.etapa_atual || "");
    let idx = etapasArr.findIndex((e) => String(e?.codigo || "").toUpperCase() === "REDE" || String(e?.nome || e?.title || "").toUpperCase().includes("REDE"));
    if (idx < 0 && prefer) idx = etapasArr.findIndex((e) => String(e?.codigo || e?.key || "") === prefer);
    if (idx < 0) idx = 0;
    ...
    const nextEtapas = etapasArr.map((et, i) => {
      if (i !== idx) return et;
      const regs = Array.isArray(et?.registros) ? et.registros : [];
      const cleaned = regs.filter((r) => !String(r?.id || "").startsWith("suas_"));
      const merged = [...sorted, ...cleaned].slice(0, 50);

      const existingUr = et?.ultimo_registro || null;
      const firstExtra = merged[0] || null;
      const tExisting = existingUr?.data_hora ? new Date(existingUr.data_hora).getTime() : 0;
      const tExtra = firstExtra?.data_hora ? new Date(firstExtra.data_hora).getTime() : 0;
      const bestUr = tExtra && (!tExisting || tExtra > tExisting) ? firstExtra : existingUr;

      return { ...et, registros: merged, ultimo_registro: bestUr || et?.ultimo_registro };
    });

    return { ...linhaMetro, etapas: nextEtapas };
  }, [linhaMetro, suasCaseUIMemo, selectedCaso?.etapa_atual]);

  // ✅ sincroniza para módulo (failsafe): sub-telas fora do escopo podem ler isso sem quebrar
  useEffect(() => {
    try {
      suasCaseUI = suasCaseUIMemo;
      linhaMetroUI = linhaMetroUIMemo;
    } catch {
      // ignore
    }
  }, [suasCaseUIMemo, linhaMetroUIMemo]);
  ...
  // ✅ Deep link: Encaminhamentos SUAS → abrir caso PopRua com 1 clique
  const openCaseRef = useRef({ carregarCasos: null });
  ...
  // Mantém referência atualizada para abrir caso a partir do card Encaminhamentos SUAS
  useEffect(() => {
    openCaseRef.current.carregarCasos = carregarCasos;
  });

  useEffect(() => {
    const handler = (ev) => {
      const caseId = toIntId(ev?.detail?.caseId);
      if (!caseId) return;

      // abre diretamente em Atendimento → Casos
      setActiveTab("atendimento");
      setAtendimentoView("casos");

      try {
        openCaseRef.current.carregarCasos?.(caseId);
      } catch (e) {
        console.error(e);
      }
    };

    window.addEventListener("poprua_open_caso", handler);

    // Deep-link a partir de outros módulos (ex.: Gestão → Abrir caso)
    try {
      const raw = localStorage.getItem("poprua_open_case_id") || "";
      if (raw) {
        localStorage.removeItem("poprua_open_case_id");
        const caseId = toIntId(raw);
        if (caseId) {
          setTimeout(() => {
            try {
              handler({ detail: { caseId } });
            } catch (e) {
              console.error(e);
            }
          }, 180);
    ...
    return () => window.removeEventListener("poprua_open_caso", handler);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  async function carregarCasos(preferSelectId = null) {
    try {
      const base = incluirInativosCasos
        ? `${API_BASE}/casos/?incluir_inativos=true`
        : `${API_BASE}/casos/`;
      const sep = base.includes("?") ? "&" : "?";
      const url = `${base}${sep}limit=200&offset=0`;
  ...
  async function selecionarCaso(casoLike) {
    ...
    // ✅ quando não está na lista (paginação), busca no backend
    if (!casoObj) {
      try {
        const r = await apiFetch(`${API_BASE}/casos/${casoId}`);
        if (r.ok) {
          casoObj = await r.json();
        }
      } catch (e) {
        console.error(e);
      }
          ...
          <nav className="app-tabs" aria-label="Navegação principal">
            {podeVerAba("inicio") && (
              <button
                type="button"
                className={
                  "app-tab" + (activeTab === "inicio" ? " app-tab-active" : "")
                }
            ...
            {podeVerAba("atendimento") && (
              <button
                ...
                className={
                  "app-tab" +
                  (activeTab === "atendimento" ? " app-tab-active" : "")
                }
                onClick={() => {
                  setActiveTab("atendimento");
                  setAtendimentoView("casos");
            ...
            {podeVerAba("pessoas") && (
              <button
                ...
                className={
                  "app-tab" +
                  (activeTab === "pessoas" ? " app-tab-active" : "")
                }
                onClick={() => {
                  setActiveTab("pessoas");
                  setPessoasView("ficha");
            ...
            {podeVerAba("saude_intersetorial") && (
              <button
                ...
                className={
                  "app-tab" +
                  (activeTab === "saude_intersetorial" ? " app-tab-active" : "")
                }
                onClick={() => {
                  // se vier da navegação, tenta abrir já no caso selecionado
                  setSaudeCasoInicialId(
                    selectedCaso?.id != null ? String(selectedCaso.id) : ""
                  );
                  setActiveTab("saude_intersetorial");
                }}
              >
            ...
            {podeVerAba("familia") && (
              <button
                type="button"
                className={
                  "app-tab" + (activeTab === "familia" ? " app-tab-active" : "")
                }
            ...
            {podeVerAba("dashboard") && (
              <button
                ...
                className={
                  "app-tab" +
                  (activeTab === "dashboard" ? " app-tab-active" : "")
                }
            ...
            {podeVerAba("guia_suas") && (
              <button
                type="button"
                className={
                  "app-tab" +
                  (activeTab === "guia_suas" ? " app-tab-active" : "")
                }
                onClick={() => setActiveTab("guia_suas")}
                title="Conteúdos práticos para equipe e gestão: financiamento, organização e equipamentos do SUAS."
              >
                Guia SUAS
              </button>
            )}
            ...
            {podeVerAba("regional") && (
              <button
                ...
                className={
                  "app-tab" +
                  (activeTab === "regional" ? " app-tab-active" : "")
                }
            ...
            {podeVerAba("protocolo") && (
              <button
                type="button"
                className={
                  "app-tab" + (activeTab === "protocolo" ? " app-tab-active" : "")
                }
                onClick={() => {
                  // abre já no caso selecionado, se existir
        ...
        {activeTab === "inicio" && (
          <TelaInicialSistema
            ...
            navegar={{
              ...
              encaminhamentos: () => setActiveTab("encaminhamentos"),
...
carregarLinhaMetro={carregarLinhaMetro}
  selectedCaso={selectedCaso}
            linhaMetro={linhaMetroUIMemo || linhaMetro}
            suasCaseUI={suasCaseUIMemo}
        ...
        {activeTab === "saude_intersetorial" && (
          <div className="layout-1col">
            <PageHero
              ...
              title="Pop Rua — Saúde intersetorial"
              subtitle="Encaminhe para UBS/CAPS/Hospital/Consultório na Rua com exposição mínima de dados."
              tips={[
                "Compartilhe apenas o mínimo necessário (LGPD).",
                "Acompanhe status e devolutiva no histórico do caso.",
                "Use como evidência para rede e gestão.",
              ]}
              badge="POP RUA"
            />
        ...
        {activeTab === "encaminhamentos" && (
          <div className="layout-1col">
            <PageHero
              kicker="MÓDULO SUAS · POP RUA EM REDE"
              title="Pop Rua — Encaminhamentos intermunicipais"
              subtitle="Envio, recebimento e devolutivas entre municípios, com rastreabilidade e SLA."
              tips={[
                "Inbox por origem/destino e status por etapa.",
                "Contrarreferência e recebimento com documentos automáticos.",
                "Vincule ao caso para histórico completo.",
              ]}
              badge="POP RUA"
              rightText={
                <>
        ...
        {activeTab === "encaminhamento_intermunicipal" && (
          <div className="layout-1col">
            <PageHero
              kicker="MÓDULO SUAS · POP RUA EM REDE"
              title="Pop Rua — Novo encaminhamento intermunicipal"
              subtitle="Preencha dados mínimos e acompanhe o fluxo por etapa (origem/destino), com LGPD aplicada."
              tips={[
                "Compartilhe o mínimo necessário (LGPD).",
                "Defina prazo e observações para orientar o destino.",
                "Acompanhe na Linha do metrô e Inbox (origem/destino).",
              ]}
              badge="POP RUA"
              actions={
                <button
...
function TelaCasos({
  ...
  selectedCaso,
  linhaMetro,
  // ✅ Integração SUAS (devolutivas/cobranças) — calculada no App (useMemo)
  suasCaseUI,
  atendimentos,
  pessoaIdForm,
apiFetch,
...
carregarLinhaMetro,
  ...
  // ✅ performance: casos por pessoa são carregados sob demanda (evita carregar milhares de casos no front)
  const [casosPessoa, setCasosPessoa] = useState([]);
  const [casosPessoaTotal, setCasosPessoaTotal] = useState(null);
  const [casosPessoaLoading, setCasosPessoaLoading] = useState(false);
  const [casosPessoaOffset, setCasosPessoaOffset] = useState(0);

  async function carregarCasosDaPessoa(pid, opts = {}) {
    const pessoaId = Number(pid);
    if (!pessoaId) {
      setCasosPessoa([]);
      setCasosPessoaTotal(null);
      setCasosPessoaOffset(0);
      return;
    }

    const append = !!opts.append;
    const offset = append ? Number(casosPessoaOffset || 0) : 0;

    try {
      setCasosPessoaLoading(true);
      const base = incluirInativosCasos
        ? `${API_BASE}/casos/?incluir_inativos=true`
        : `${API_BASE}/casos/`;
      const sep = base.includes("?") ? "&" : "?";
      const url = `${base}${sep}pessoa_id=${encodeURIComponent(pessoaId)}&limit=50&offset=${encodeURIComponent(offset)}`;

      const res = await apiFetch(url);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);

      const totalH = res.headers.get("x-total-count") || res.headers.get("X-Total-Count") || null;
      const totalN = totalH != null && String(totalH).trim() !== "" ? Number(totalH) : null;
      ...
      setCasosPessoaTotal(totalN);
      if (append) {
        setCasosPessoa((prev) => {
          const seen = new Set((prev || []).map((x) => Number(x && x.id)));
          const merged = [...(prev || [])];
        ...
        setCasosPessoaOffset(offset + normalizados.length);
      } else {
        setCasosPessoa(normalizados);
        setCasosPessoaOffset(normalizados.length);
      }
    } catch (e) {
      console.error(e);
      // não derruba a tela: só esvazia
      if (!opts.append) setCasosPessoa([]);
    } finally {
      setCasosPessoaLoading(false);
    }
  }
  ...
  useEffect(() => {
    // carrega casos da pessoa sob demanda (server-side)
    if (!pessoaRefId) {
      setCasosPessoa([]);
      setCasosPessoaTotal(null);
      setCasosPessoaOffset(0);
    ...
    carregarCasosDaPessoa(pessoaRefId, { append: false });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pessoaRefId, incluirInativosCasos]);

  const casosDaPessoa = useMemo(() => {
    return (casosPessoa || []).slice().sort((a, b) => Number(b?.id) - Number(a?.id));
  }, [casosPessoa]);
  ...
  useEffect(() => {
    ...
    const maisRecente = (casosPessoa || [])
      .slice()
      .sort((a, b) => Number(b?.id) - Number(a?.id))[0];

    if (maisRecente) selecionarCaso(maisRecente);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pessoaVerCasosId, casosPessoa]);
  ...
  return (
    <>
      ...
      <PageHero
        kicker="MÓDULO SUAS · POP RUA EM REDE"
        title="Pop Rua — Atendimento"
        subtitle="Selecione uma pessoa, escolha um caso e registre atendimentos com padronização, rastreabilidade e LGPD."
        tips={[
          "Clique na Linha do metrô para ver histórico por etapa.",
          "Use Registrar atendimento para criar um registro rápido.",
          "Pendências e SLA alimentam a Gestão automaticamente.",
        ]}
        badge="POP RUA"
        rightText={
          <>
        ...
        <div className="card casos-card-lista">
          <div className="card-header-row">
            ...
            <button
              ...
              onClick={() => {
                      carregarCasos();
                      if (pessoaRefId) carregarCasosDaPessoa(pessoaRefId, { append: false });
                    }}
            >
              Atualizar
            </button>
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center", marginTop: 10, justifyContent: "space-between", flexWrap: "wrap" }}>
            <span className="texto-suave">
              {casosPessoaTotal != null
                ? `Mostrando ${casosDaPessoa.length} de ${casosPessoaTotal}`
                : `Mostrando ${casosDaPessoa.length}`}
            </span>

            {pessoaRefId && casosPessoaTotal != null && casosPessoaOffset < casosPessoaTotal && (
              <button
                type="button"
                className="btn btn-secundario btn-secundario-mini"
                onClick={() => carregarCasosDaPessoa(pessoaRefId, { append: true })}
                disabled={casosPessoaLoading}
              >
                {casosPessoaLoading ? "Carregando..." : "Carregar mais"}
              </button>
            )}
          </div>
        </div>
          ...
          <>
            ...
            {abaDetalhe === "linha" && (
              <div className="card casos-content-card">
                {(suasCaseUI?.timeline || []).length ? (
                  <div className="card" style={{ padding: 12, borderRadius: 18, boxShadow: "none", border: "1px solid rgba(2,6,23,.06)", marginBottom: 12 }}>
                    <div style={{ fontWeight: 900, marginBottom: 8 }}>Devolutivas / cobranças (SUAS)</div>
                    <div className="texto-suave" style={{ marginBottom: 10 }}>
                      Quando outro equipamento responder, aparece aqui automaticamente.
                    </div>
                    <div style={{ display: "grid", gap: 8 }}>
                      {suasCaseUI.timeline.slice(0, 5).map((t) => (
                        <div key={t.id} className="card" style={{ padding: 10, borderRadius: 14, boxShadow: "none", border: "1px solid rgba(2,6,23,.06)" }}>
                          <div style={{ fontWeight: 900 }}>{t.texto}</div>
                          <div className="texto-suave">
                            {t.por} · {t.criado_em ? new Date(t.criado_em).toLocaleString("pt-BR") : "—"}
                          </div>
                          {t.detalhe ? (
...
function LinhaMetro({ linhaMetro, casoId, apiBase, apiFetch, atendimentos = [], municipios = [], onRefresh }) {
  ...
  function nomeMunicipio(mId) {
    const id = Number(mId);
    const m = (municipios || []).find((x) => Number(x && x.id) === id);
...
function TelaFichaAtendimento(props) {
  ...
  return (
    <div className="layout-1col">
      <PageHero
        kicker="MÓDULO SUAS · POP RUA EM REDE"
        title="Pop Rua — Ficha de atendimento"
        subtitle="Registre um atendimento vinculado a uma pessoa e, se desejar, a um caso Pop Rua em andamento."
        tips={[
          "Padronize tipo/local/equipamento para qualificar dados.",
          "Vincule a um caso quando houver (melhor rastreabilidade).",
          "O registro aparece na timeline do caso e nos relatórios.",
        ]}
        badge="POP RUA"
      />
...
function TelaCadastroPessoa(props) {
  ...
  return (
    <div className="layout-1col cadastro-shell">
      <PageHero
        kicker="MÓDULO SUAS · POP RUA EM REDE"
        title="Pop Rua — Novo cadastro de pessoa"
        subtitle="Registre os dados mínimos para atendimento. Campos sensíveis só quando necessários e por perfis autorizados."
        tips={[
          "Cadastro mínimo (nome social + referência).",
          "LGPD: evitar dado sensível sem necessidade.",
          "Complete depois na Ficha da pessoa (sem travar o atendimento).",
        ]}
        badge="POP RUA"
        rightText={
          <>
...
function TelaDashboardPopRua({
  ...
  return (
    <div className="layout-1col">
      <PageHero
  kicker="MÓDULO SUAS · POP RUA EM REDE"
  title="Pop Rua — Dashboard"
  subtitle="Indicadores do município, casos por etapa e acompanhamento operacional."
  tips={[
    "KPIs por etapa e status (em andamento/encerrados/arquivados).",
    "Acompanhe volume e gargalos do atendimento.",
    "Use Atualizar para recalcular indicadores.",
  ]}
  badge="POP RUA"
  rightText={
    "Perfil: " +
    String(perfilAtivo || "—") +
    (municipioAtivoNome ? " · Município: " + municipioAtivoNome : "")
  }
  actions={
          ...
          <ul className="dashboard-stage-list">
            {porEtapa.slice(0, 10).map(([etapa, qtd]) => (
              <li key={String(etapa)} className="dashboard-stage-row">
                ...
                <div className="dashboard-progress-track">
                  <div
                    className="dashboard-progress-fill andamento"
                    style={{ width: totalCasos ? `${Math.round((qtd / totalCasos) * 100)}%` : "0%" }}
                  />
                </div>
              </li>
            ))}
